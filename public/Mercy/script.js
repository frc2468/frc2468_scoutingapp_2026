  ...
  try {
    ...
    //loops through to check types
      for(var a in Object.keys(data))
      {
        ...
        if(Number.isFinite(data[labels.indexOf("Position")])){
          document.getElementById("status").innerHTML +=
          `Failed Upload for ${data[labels.indexOf("Position")]}: index ${i}: type should be a ${typeof(dataStructure.getDataValues()[i])}, but is a ${/^\d+$/.test(data[i])?'number':'string'} <br><br>`;
          broke = true;
        }

        if(Number.isFinite(data[labels.indexOf("Match")])){
          document.getElementById("status").innerHTML +=
          `Failed Upload for ${data[labels.indexOf("Match")]}: index ${i}: type should be a ${typeof(dataStructure.getDataValues()[i])}, but is a ${/^\d+$/.test(data[i])?'number':'string'} <br><br>`;
          broke = true;
        }
