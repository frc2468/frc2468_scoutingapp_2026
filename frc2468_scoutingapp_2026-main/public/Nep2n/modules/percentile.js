...
export class Percentile {
    ...
    sortPercentile() {
        ...
        for(let i=0; i<keys.length; i++){
            let sorted_arr = []
            // Keep one entry per robot — coerce missing/non-numeric to 0
            for(let j=1; j<this.percentileObject.length; j++){
                ...
                if (v === true) {
                    ...
                    // preserve column by treating undefined/non-numeric as 0
                    num = 0;
                }
                sorted_arr.push(num);
            }
    ...
    findPercentileOf(val, name) {
        // Guard: name must exist and val must be a number
        if (!name || !this.percentileObjectSorted[name] || this.percentileObjectSorted[name].length === 0) return 0;
